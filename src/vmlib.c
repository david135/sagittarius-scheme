...
static SgObject _sagittarius_vm_call_macro_expander(SgObject *args, int argc, void *data_)
{
  ...
  {
    SgObject SG_RETURN = SG_UNDEF;
    SG_RETURN = (Sg_Apply(macro->transformer, SG_LIST4(macro, expr, p1env, macro->data)));
    return SG_RETURN;
  }
}
...
;
                if ((SG_IDENTIFIERP(SG_CAR(vp)) && SG_IDENTIFIERP(oname))) {
                  {
                    SgObject e1 = SG_IDENTIFIER_ENVS(oname);
                    SgObject e2 = SG_IDENTIFIER_ENVS(SG_CAR(vp));
                    SgObject lam1 = Sg_Assq(oname, e1);
                    SgObject lam2 = Sg_Assq(SG_CAR(vp), e2);
                    if (!(SG_FALSEP(lam1))) {
                      lam1=SG_CDR(lam1);
                    }
;
...
;
                    if ((SG_EQ(oname, SG_CAR(vp)) && SG_EQ(lam1, lam2))) {
                      return SG_CDR(vp);
                    }
;
                  }
