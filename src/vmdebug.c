...
static SgObject vmdebug_vm_dump_code(SgObject *SG_FP, int SG_ARGC, void *data_)
{
  ...
  if (!SG_CODE_BUILDERP(cb_scm))
    Sg_WrongTypeOfArgumentViolation(
       sg__rc.d80[0], SG_MAKE_STRING("code-builder"), cb_scm, SG_NIL);
...
SG_RESULT=(Sg_Assq(o,(Sg_VM())->history));
SG_RETURN(SG_OBJ_SAFE(SG_RESULT));
}
  }
}
...
void Sg__Init_sagittarius_vm_debug () {
  ...
  sg__rc.d80[0] = SG_MAKE_STRING("vm-dump-code");
  Sg_InsertBinding(lib, Sg_Intern(sg__rc.d80[0]), &vmdebug_vm_dump_code__STUB);
  SG_PROCEDURE_NAME(&vmdebug_vm_dump_code__STUB) = sg__rc.d80[0];
  sg__rc.d80[1] = SG_MAKE_STRING("source-info");
  Sg_InsertBinding(lib, Sg_Intern(sg__rc.d80[1]), &vmdebug_source_info__STUB);
  SG_PROCEDURE_NAME(&vmdebug_source_info__STUB) = sg__rc.d80[1];
  sg__rc.d80[2] = SG_MAKE_STRING("source-info-set!");
  Sg_InsertBinding(lib, Sg_Intern(sg__rc.d80[2]), &vmdebug_source_info_setX__STUB);
  SG_PROCEDURE_NAME(&vmdebug_source_info_setX__STUB) = sg__rc.d80[2];
  sg__rc.d80[3] = SG_MAKE_STRING("lookup-expansion-history");
  Sg_InsertBinding(lib, Sg_Intern(sg__rc.d80[3]), &vmdebug_lookup_expansion_history__STUB);
  SG_PROCEDURE_NAME(&vmdebug_lookup_expansion_history__STUB) = sg__rc.d80[3];
  sg__rc.d80[4] = SG_MAKE_STRING("save-expansion-history!");
  Sg_InsertBinding(lib, Sg_Intern(sg__rc.d80[4]), &vmdebug_save_expansion_historyX__STUB);
  SG_PROCEDURE_NAME(&vmdebug_save_expansion_historyX__STUB) = sg__rc.d80[4];
}
