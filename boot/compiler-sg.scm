...
(define
 ...
 (lambda
  ...
  (define
   ...
   (lambda
    (e)
    (and (pair? e) (pair? (cdr e)) (null? (cddr e)) (quote? (car e)))))
  (define
   constant?
   (lambda
    (e)
    ...
    (cond
     ((pair? expr)
      (if
       ...
       (smatch
        expr
        ((((? unquote? -) e1 ___) . e2) (emit-cons* e1 (expand e2 0)))
        ...
        (head
         (unless
          ...
          (syntax-error
           ...
           (caar exprs)))
         (cond
          ((lvar? head) (pass1/body-finish intdefs intmacros exprs p1env))
          ((macro? head)
           (pass1/body-macro-expand-rec head exprs intdefs intmacros p1env))
          ((syntax? head) (pass1/body-finish intdefs intmacros exprs p1env))
          ...
          ((and
            (vm-r6rs-mode?)
            (or
             (global-eq? head 'let-syntax p1env)
...
(define
 pass1/body-macro-expand-rec
 (lambda
  (mac exprs intdefs intmacros p1env)
  (pass1/body-rec
   (acons
...
(define
 ...
 (lambda
  ...
  (cond
   ((and (null? intdefs) (null? intmacros)) (pass1/body-rest exprs p1env))
   ((null? intmacros)
    (let*
     ...
     (define
      ...
      (lambda
       ...
       (let*
        ...
        newenv)))
     (define
      letrec-syntax-parser
      (lambda
       (exprs p1env)
       (let*
        ((newenv (p1env-extend p1env exprs LEXICAL))
         (trans
          (map
           (lambda
            (n x)
        ...
        (let
         ...
         (if
          ...
          (case
           (caar exprs)
           ((def rec)
            (loop (cdr exprs) (letrec-syntax-parser (cdar exprs) env)))
           ((let)
            (loop (cdr exprs) (let-syntax-parser (cdar exprs) env))))))))
